...
#  print("NUDEBUG: INPUT={}".format(text))
  if forcecolor is not None:
    colorwith=str(int(forcecolor) % 16000)
  else:
    colorwith=str(cachehash(text) % 16000)
  commandseq = '\033[38;5;' + colorwith + 'm'
  if (cachehash(text) > 16000):
    commandseq=commandseq + "\033[5;7m"
  return str(commandseq + text + '\033[0m')
...
#    lambda s: "DVA[%s]=<%s:%s:%s>".format(colorwrap(s.group(0),cachehash(s.group(0))),colorwrap(s.group(1),cachehash(s.group(1))),colorwrap(s.group(2),cachehash(s.group(2))),colorwrap(s.group(3),cachehash(s.group(3)))
    ...
    line = re.sub(r"([0-9a-f]+)L/([0-9a-f]+)P",lambda s: "{}L/{}P".format(colorwrap(s.group(1)),colorwrap(s.group(2))),line)
    line = re.sub(r"([0-9a-f]+)L",lambda s: "{}L".format(colorwrap(s.group(1))),line)
